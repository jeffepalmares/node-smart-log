...
class LoggerBuilder {
        ...
        if (item instanceof TypeError || item instanceof Error) {
            ...
            Object.keys(item).forEach((key) => {
                if (LoggerBuilder.shouldHide(key) && !(process.env.SHOW_SENSETIVE_LOG == 'true')) {
                    info[key] = constants_2.LoggerConstants.HidenLogFieldLabel;
                }
                else {
                    info[key] = item[key];
                }
    ...
    static getLogger(custom) {
        return process.env.NODE_ENV === constants_1.Environment.LOCAL ? LoggerBuilder.createLocalWinstonLogger(custom) : LoggerBuilder.createDefaultWinstonLogger(custom);
    }
    static getLogLevel() {
